...
)
{
	...
	if (funKind == FunctionType::Kind::External || funKind == FunctionType::Kind::DelegateCall)
	{
		templ(
			...
			m_context.variablePart(_functionCall.expression(), 1) +
			")) { revert(0, 0)} }"
		);
		existenceChecked = true;
	}
	else
		templ("existenceCheck", "");
	...
	{
		solAssert(haveReturndatacopy, "");
		templ("copyDynamicReturnData", "returndatacopy(" + pos + ", 0, returndatasize()) }");
	}
	...
	if (returnTypes.empty())
		...
		templ("defineRetVars", "let " + m_context.variable(_functionCall) + " := ");
}
